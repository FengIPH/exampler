...
class VarMF_xij_sig(nn.Module):

    def __init__(self, config):
        super(VarMF_xij_sig, self).__init__()
        self.num_users = config['num_users']
        self.num_items = config['num_items']
        self.num_xij = config['num_xij']
        self.latent_dim = config['latent_dim_var']
        self.xij_dim = config['latent_dim_var']
        self.embedding_user = torch.nn.Embedding(num_embeddings=self.num_users, embedding_dim=self.latent_dim)
        self.embedding_item = torch.nn.Embedding(num_embeddings=self.num_items, embedding_dim=self.latent_dim)
        self.embedding_xij = torch.nn.Embedding(num_embeddings=self.num_xij, embedding_dim=self.xij_dim)
        self.sig = nn.Sigmoid()
    ...
    def forward(self, users, items, xij):
        ...
        items_emb = self.embedding_item(items.long())
        xij_emb = self.embedding_xij.weight.repeat(len(users), 1)
        xij_emb = xij_emb * (xij.reshape(-1, 1))

        inner_pro1 = torch.mul(users_emb, items_emb)
        inner_pro2 = torch.mul(users_emb, xij_emb)
        inner_pro3 = torch.mul(items_emb, xij_emb)

        rating1 = torch.sum(inner_pro1, dim=1)
        rating2 = torch.sum(inner_pro2, dim=1)
        rating3 = torch.sum(inner_pro3, dim=1)

        rating = self.sig(rating1 + 0.5*rating2 + 0.5*rating3)
